...
- **⚠ Для правок «на месте» использовать `perl -pi -e`, а НЕ `sed`.** Платформа — darwin (BSD sed) плюс CRLF в рабочем дереве, и это даёт две ловушки с *тихим* отказом:
  1. **Якорь `$` не совпадает.** Строка заканчивается на `\r`, поэтому `sed 's/^package okr$/package progress/'` не срабатывает и НЕ сообщает об ошибке.
  2. **BSD sed не поддерживает `\b`.** `sed 's/\bokr\./progress./g'` — молчаливый no-op (проверено: `printf 'okr.Foo' | sed 's/\bokr\./XX./g'` возвращает `okr.Foo` без изменений).

  `perl -pi -e` свободен от обеих: поддерживает `\b`, а `^`/`$` ведут себя предсказуемо, и переводы строк сохраняются. Замены import path по строке в кавычках (`'|"okrs/internal/x"|"okrs/internal/y/x"|'`) безопасны и в sed — там нет ни якорей, ни `\b`, — но ради единообразия везде используется perl.
