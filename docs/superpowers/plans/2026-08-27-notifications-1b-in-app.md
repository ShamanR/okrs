...
- [ ] **Шаг 7: Обновить golden-тест маршрутов**

Запустить: `go test ./internal/http -run RoutesGolden -update-routes && git diff --stat internal/http/testdata/routes.golden`
Ожидается: в golden добавились ровно три строки — `GET /api/v1/notifications`, `GET /api/v1/notifications/unread-count`, `POST /api/v1/notifications/read`. Больше ничего не изменилось.
