...
22 структуры, по одной на каждую существующую `ActivityAction`:

`GoalCreated`, `GoalDeleted`, `GoalCopied`, `GoalMoved`, `GoalFieldsChanged`, `GoalOwnerChanged`, `GoalShared`, `GoalUnshared`, `GoalLinked`, `GoalUnlinked`, `KRCreated`, `KRDeleted`, `KRFieldsChanged`, `KRProgressUpdated`, `KRNoteUpdated`, `StatusChanged`, `CommentAdded`, `CommentResolved`, `CommentReopened`, `CommentDeleted`, `ReplyAdded`, `ReplyDeleted`.
...
| `kr_progress` | скоупный | `KRProgressUpdated` | да |

Подписчик уведомлений регистрируется ровно на эти 13 типов событий; остальные 9 до него не доходят. Это и есть польза от гранулярной подписки.
...
- `KRNoteUpdated` — заметка к KR отнесена в коде к категории `discussion` ([`usecase/keyresult/keyresult.go:213`](../../../internal/usecase/keyresult/keyresult.go)), то есть продукт уже классифицирует её как обсуждение, а не как изменение состава. Тянуть её в `goal_changed` значило бы разойтись с собственной классификацией;
...
- **канал, оставшийся в БД, но не подключённый в этой сборке** (выключили опцию, откатили бинарь) — не ошибка: строки доставки получают статус `skipped` с причиной «канал недоступен в этой сборке», админка показывает карточку как неактивную. Данные не теряются, при возврате канала всё оживает.
...
- **гейт по entitlements** — по имени канала: `entitlement.notifications.<Descriptor.Name>`. Внешний канал гейтится тем же механизмом без правок ядра. `in_app` не гейтится никогда. Кто и как проставляет эти ключи — см. §10.5.
...
Администратор пространства **не может** расширить список доступных ему каналов — только распорядиться выданным. Разрешение выдаётся вручную системным администратором через `PUT /api/v1/system/tenants/{id}/entitlements`. В OSS-реализации `unlimited` разрешено всё, и уровень фактически прозрачен.
...
1. `PUT /api/v1/admin/settings/notifications/channels/{ch}` для неразрешённого канала → `403`, независимо от того, что показал UI;
...
PUT    /api/v1/admin/settings/notifications/channels/{ch}
POST   /api/v1/admin/settings/notifications/channels/{ch}/test
...
- состояние `hciData` / `hciOpen`, запрос health check-in и компонент `HealthCheckInPanel` остаются в коде, но **точки входа у них больше нет** — панель временно недоступна пользователю. Это осознанное временное состояние: новое место для входа в Health Check-in определяется отдельной задачей, вне этой спеки;
...
Оставлять недостижимый код надолго репозиторий не любит (в слоистом рефакторинге такие пакеты вычищались), поэтому в плане фазы 1 это фиксируется как явный технический долг со сроком «до задачи о новом месте HCI».
...
Карточка на канал, **форма генерится из `Descriptor`** — админка не знает о существовании Telegram, и Slack появится в ней сам. Секрет показан как `••••4821` с кнопкой «Заменить». Плюс тумблер «Включён», кнопка «Проверить» (шлёт тестовое сообщение администратору) и последняя ошибка доставки.
...
Это требование к API, а не к разметке: `GET /api/v1/admin/settings/notifications` **не отдаёт** дескрипторы неразрешённых каналов, поэтому состав сборки через тенантный API не разведать. `PUT` по неразрешённому каналу возвращает `403` — на случай прямого запроса мимо UI.

### 13.5. Системная панель

`/system` → «Entitlements» получает блок «Каналы уведомлений»: тумблер `entitlement.notifications.<name>` на каждый канал, собранный в этот бинарь. Список приходит из `GET /api/v1/system/notification-channels`, а не из захардкоженного `KNOWN_ENT` (`web/static/system.js:173`), — иначе канал из внешнего репозитория в панели не появился бы никогда.
