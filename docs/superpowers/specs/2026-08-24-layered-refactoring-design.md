...
3. **Нарушение правила 1 спеки 010** («Никакой бизнес-логики в handlers») — `hcLoader` собирает `PeriodData` из пяти репозиториев прямо в `server.go`, там же живут петля снапшотов, advisory-lock и `daysBetween`. **Устранено в этапе D:** загрузчик → `usecase/healthcheckin`, петли → `internal/scheduler`. Побочный выигрыш: `Routes()` перестал запускать goroutine и стал чистой сборкой, пригодной для теста без БД.
...
**Usecase зависит от сервисов, а не от репозиториев.** Это решение принято на этапе D и стоило +37 однострочных методов в сервисах сущностей: сценарии делали ~50 различных вызовов репозиториев, и большинство этих операций (`CreateGoal`, `ReplaceGoalShares`, `SetTeamPeriodStatus`) — операции над одной сущностью, которым место в её сервисе. Альтернатива (usecase → repo) была дешевле, но оставляла у store две двери и делала формулировку «оркестрация сервисов» неправдой.

**Зависимости между usecase-пакетами запрещены.** Где сценарию нужен результат другого, объявляется узкий порт на стороне потребителя: `export.BoardReader` (реализует `*okrboard.UseCase`), `goallink.GoalProgressReader` (реализует `*goal.Service`), три порта в `scheduler`. Приём применён трижды и зафиксирован в спеке 010.
...
**Инвариант всех этапов:** множество зарегистрированных роутов (метод + путь) не меняется. Проверяется `internal/http/routes_golden_test.go`: `chi.Walk` по собранному роутеру против `testdata/routes.golden`. Grep по исходникам для этого не годится — как только URI становится переменной (табличная регистрация SSR-страниц), он молча перестаёт их видеть.

**Результат.** Все пять этапов закрыты. 163 пакета в модуле (было 55), из них 103 — handlers. `server.go` — 579 строк (было 701) и только монтирует. `go build ./...`, `go vet ./...`, `go test ./...` — зелёные, 142 роута совпадают с эталоном.
...
| `specs/070-code-structure.md` | E | **Новая.** Карта «URI → пакет handler'а» (142 роута, 81 пакет), карта service/usecase, правила разложения для новых эндпоинтов, два намеренных исключения (`/static/*`, `/login` в disabled-режиме) |
...
## 13. Отложено за скоуп (TODO)

Помечено `TODO(refactoring)` в коде. Ничего из этого не относится к разделению слоёв — это долг, который рефакторинг вскрыл или создал.

Закрыто по ходу этапа E:

- **Суффикс `_moved_test.go`** — 12 файлов переименованы по содержанию, комментарий о происхождении оставлен одной строкой.
- **Экспорт «ради фасада»** (`team.BuildHierarchy`, `FindDirectChildren`, `CollectDescendantIDs`, `HierarchyFromTeams`, `healthcheckin.BuildTeamPath`, `Abs`) — проверено после снятия фасада: у каждой функции остался внешний потребитель (`usecase/okrboard`, `usecase/period`). Сужать видимость не нужно, комментарий в `service/team/team.go` заменён на этот вывод.
- **`admin/handler_test.go` не соответствует gofmt** — файл удалён вместе с фасадным обработчиком; его тесты разнесены по пакетам-владельцам URI.
- **Курсор ленты активности в handler-слое** — кодек `created_at|id` переехал в `service/activity` (`Filter`/`Page`), `activitycommon` больше не импортирует store.
- **Разнобой в именах файлов обработчиков** — 84 файла переименованы в `handler.go`, 16 тестов — в `handler_test.go`; конвенция записана в `specs/070-code-structure.md`.
- **Handler-пакеты без тестов** — было 64 из 89, стало 0. Общая обвязка — `internal/http/handlers/handlertest`.

Осталось открытым:

| Что | Где | Почему отложено | Как закрыть |
|---|---|---|---|
| Store-типы в handlers: sentinel-ошибки, `*Input`, read-модели | `goals.ErrNotFound`, `teams.TeamInput`, `periods.PeriodInput`, `memberships.MembershipWithTenant`, `users.TenantUser`, `grants.HierarchyGrant` | Курсор ленты активности вычищен (это была работа с сырыми данными), остальное — имена типов: handler их не конструирует и не интерпретирует, а передаёт насквозь | Sentinel-ошибки перенести в `core/domain` (там уже есть `errors.go`); `*Input` заменить типами сервисов; read-модели — DTO ответа |
| Два конверта ошибки в API | `v1.WriteError` против `admincommon`/`systemcommon`/`onboardingcommon.WriteError` | Приведение к одному формату ломает контракт: нужен синхронный фикс фронтенда. 41 файл в структурном конверте, 22 — в плоском | Перевести admin/system/onboarding на `v1.WriteError`, завести коды ошибок, обновить клиент. Описано в `specs/040-api-contract.md` |
| Экспорт полей и типов-аргументов `servicetest.GoalStore` | `internal/service/servicetest/goalstore.go` | Фейк понадобился трём пакетам сразу; экспорт — цена совместного использования | Конструкторы-билдеры (`NewGoalStore().WithGoals(...)`) вместо публичных полей |
| Адаптер `rawDeps`/`newFromRepos` | `internal/usecase/{goal,keyresult}` | Тесты писались против repo-образных `Deps`; переписывать 60 тел не входило в рефакторинг | Переписать тесты на `Deps` с сервисами, адаптер удалить |
| Дубль `newOnboardingForTest` | `internal/service/provisioning/provisioning_test.go` | Go не делит `_test.go`-хелперы между пакетами | `servicetest/db.go` с фикстурами поверх настоящего pool |
| 4 файла не соответствуют gofmt | `goals/handler.go`, `authhandler/handler.go`, `progress/progress_test.go`, `teams/teams_test.go` | Предсуществующее; правка = unrelated churn в диффе рефакторинга | `gofmt -w` по одному файлу отдельным коммитом |
| Сборка графа зависимостей живёт в `internal/http/httpdeps`, а не в `app.New` | `internal/http/httpdeps/httpdeps.go` | Обработчикам нужны 17 полей; протаскивать их через `app` — параметр ради параметра. `httpdeps` уже является composition root и не тянет HTTP-специфику | Решать, если появится второй транспорт (gRPC/CLI), которому нужен тот же граф |
| Две реализации `UserSelector` | `tracker.js`, `admin.js` | Фронтенд, к слоям отношения не имеет; уже числится в спеке 010 как долг | Параметризуемый общий компонент |
| Базовый образ Docker поднят до `golang:1.25-alpine` | `Dockerfile` | **Уже сделано**, но выходит за пять пунктов задания: образ не собирался (go.mod требует 1.25) | Решение заказчика: оставить или откатить |
