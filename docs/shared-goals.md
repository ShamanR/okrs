...
Нажатие «Удалить» на общей цели **открепляет от неё текущую команду** (ту, в контексте которой нажата кнопка):

- **Если цель общая (разделена с другими командами):** удаляется только привязка вашей команды — у остальных команд-участников цель остаётся. Если ваша команда была владельцем, владение переходит к одной из оставшихся команд-участников.
- **Если цель не общая (только у вашей команды):** цель удаляется целиком, вместе со всеми Key Results.

Удалить общую цель целиком можно, только открепив от неё все команды.
...
Открепится только эта команда — у остальных участников цель сохранится. Если открепляется команда-владелец, владение переходит к одной из оставшихся команд. Цель удаляется целиком только тогда, когда от неё откреплена последняя команда.
