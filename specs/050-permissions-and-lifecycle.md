...
- если родитель скрыт по правилам visibility, но дочерняя команда видима, дочерняя команда должна автоматически стать корневой в выдаче (без скрытия из-за отсутствующего родителя).
