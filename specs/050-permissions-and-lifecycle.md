...
**Видимость (share-aware).** Лента `/activity-log` (`GET /api/v1/activity`) разграничена по тем же
правилам, что и трекер:

- разделение по тенантам — все запросы фильтруются по `tenant_id` из scope;
- событие видно, если его **audience** (owner team цели ∪ команды, которым цель расшарена; для
  событий без цели — команда события) пересекается с доступными пользователю командами
  (`PolicyEvaluator`); admin видит все события тенанта. «Вижу цель → вижу её лог»;
- нет доступа ни к одной команде из audience → событие не попадает ни в ленту, ни в счётчики
  дерева; обойти прямым запросом эндпоинта нельзя;
- **fail-closed**: событие с `team_id IS NULL` (напр. команда hard-deleted) в scoped-ленту не
  попадает;
