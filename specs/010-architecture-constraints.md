...
- `internal/http/httpdeps` — сборка графа сервисов и usecase: `Build(store, grantsCache, hcCache, bus, logger) Deps`. Единственное место, где известен полный список зависимостей; `server.go` раздаёт из него поля по пакетам. `Build` также регистрирует на переданной шине (`internal/platform/eventbus`) подписчиков — сегодня это журнал активности (`service/activity`). Отсюда порядок вызовов: `bus` создаётся и передаётся в `Build` до того, как на нём вызывают `Start` — `Subscribe`/`SubscribeAll` после `Start` паникует;
