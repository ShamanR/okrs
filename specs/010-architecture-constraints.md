...
- `internal/core/event` — доменные события OKR, по структуре на тип, чистые данные без I/O;
...
- `internal/scheduler` — фоновые петли (обновление кэша health check-in, снимки прогресса, суточная ретенция уведомлений). Запускается из `app.New`, а НЕ из `Routes()`: построение роутера должно оставаться чистой сборкой, иначе его нельзя вызвать в тесте без goroutine и БД;
...
- `internal/http/httpdeps` — сборка графа сервисов и usecase: `Build(store, grantsCache, hcCache, bus, logger) Deps`. Единственное место, где известен полный список зависимостей; `server.go` раздаёт из него поля по пакетам. `Build` также регистрирует на переданной шине (`internal/platform/eventbus`) подписчиков: синхронный журнал активности (`service/activity`) и асинхронный fan-out уведомлений (`usecase/notification`) — резолв получателей и вставка строк не должны задерживать HTTP-ответ, поэтому только этот подписчик асинхронный. Отсюда порядок вызовов: `bus` создаётся и передаётся в `Build` до того, как на нём вызывают `Start` — `Subscribe`/`SubscribeAll` после `Start` паникует;
...
**Граница service / usecase.** Метод принадлежит сервису сущности, если трогает не более одного репозитория **и** не публикует доменное событие (`internal/core/event`, через порт `Publisher`, которому у usecase удовлетворяет `*eventbus.Bus`). Иначе это usecase. Правило операционное — проверяется механически, а не на вкус: карту «метод → репозитории» строит скрипт, а публикацию события — один grep по `events.Publish`/`s.events.Publish` в пакете. Раньше критерием было «не пишет в журнал активности»; после перехода на шину usecase не пишет журнал сам, а публикует событие, которое в журнал превращает подписчик (`service/activity.Handle`, см. выше) — так что формулировка через публикацию события бьёт по тому же месту на слой ближе к тому, что реально видно в коде usecase.
