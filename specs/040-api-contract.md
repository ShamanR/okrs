...
- для любого выбранного периода — все активные команды;
- soft-deleted команды возвращаются только если у них есть goals/OKR в этом периоде;
...
- если в иерархии родитель команды не попал в выборку, но дочерняя команда попала, дочерняя команда должна остаться в ответе как root-level (без потери видимости из-за скрытого родителя).
