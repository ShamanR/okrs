...
`GET /api/v1/users?ids[]=<udid1>&ids[]=<udid2>` — возвращает пользователей с указанными UDID. Максимум 100 UDID за запрос; лишние усекаются.
...
- Используется UserSelector (tracker.js, admin.js, app.js) через режим `?q=` и комментарии через режим `?ids[]=` для загрузки только тех пользователей, что упоминаются на странице.
