...
- `GET /api/v1/admin/settings/notifications` — каналы, доступные **этому** тенанту (выданные
  entitlement'ом), и их текущая настройка → `200 { "channels": [{ "name", "title", "enabled",
  "configured", "secret_hint", "values", "fields" }] }`. Экран показывает только выданные каналы —
  ни закрытых карточек, ни апсейла (design spec §13.4); канал, которого в списке нет, для этого
  тенанта не существует.
- `PUT /api/v1/admin/settings/notifications/{channel}` — сохранить настройку одного канала; body:
  `{ "enabled": bool, "values": {...}, "secret": "<string>" }` → `204`. Пустой `secret` означает
  «не менять сохранённый», а не «стереть» — так работает форма, показывающая маску вместо значения.
  `422`, если `enabled: true` запрошен без секрета (ни нового, ни ранее сохранённого) у канала,
  которому секрет обязателен, — и такой же `422`, называющий поле, если при `enabled: true` пусто
  любое другое обязательное поле дескриптора (`required: true`). Причина одна и та же: включённая
  строка в БД, которой нечем отправлять, обнаружится только на доставке. Выключенный канал
  сохраняется с любыми пустыми полями — это черновик настроек, а не обещание работоспособности.
  `503`, если на сервере не задан `NOTIFICATIONS_SECRET_KEY`, а канал требует секрет.
- `POST /api/v1/admin/settings/notifications/{channel}/test` — отправить проверочное сообщение
  **себе** (получатель — email вызывающего администратора, из его профиля, не из тела запроса) →
  `200 { "ok": true }`. `409`, если канал ещё не сохранён; `503` с тем же текстом, что и у `PUT`,
  если на сервере не задан `NOTIFICATIONS_SECRET_KEY` (одна причина — один ответ: обе кнопки стоят
  на одной карточке); `422`, если канал отверг сохранённую конфигурацию как непригодную —
  администратор чинит это здесь же, это не сбой сервера; `502` с текстом ошибки канала, если
  внешний API отверг отправку — это единственное место в контракте, где текст ошибки канала
  уходит наружу, а не подменяется общим сообщением, потому что он описывает конфигурацию самого
  тенанта (истёкший токен, неверный адрес), а не внутренности сервера.

  **Исключение — транспортная ошибка.** Если до API канала не дошло вовсе (отказ соединения,
  таймаут, ошибка DNS), наружу уходит общий текст без адреса и причины. `base_url` задаёт сам
  администратор пространства, поэтому проброс сырого `dial tcp …: connection refused` превратил бы
  кнопку в сканер внутренней сети с оракулом: по ответу различимы открытый порт, закрытый порт и
  статус HTTP для любого внутрикластерного адреса. Обоснование проброса («текст описывает
  конфигурацию тенанта») верно для ответа внешнего API — `401`, `403`, `404` от него — и неверно
  для транспорта, который описывает сеть сервера. Полного запрета приватных диапазонов здесь нет,
  он записан отдельным пунктом технического долга. Перед отправкой в ответ из
  этого текста вычищается плейнтекст секрета тенанта (`secretMaskingSender`,
  `internal/service/notificationchannel/secretmask.go`): канал из внешнего модуля может подставить
  секрет прямо в URL (Telegram Bot API: `/bot<TOKEN>/sendMessage`), и необработанный `url.Error`
  вынес бы токен в тело `502` как есть.

Канал, недоступный пространству, и канал, которого нет в сборке, неразличимы для тенант-админа, но
механизм разный на разных маршрутах. На `PUT .../notifications/{channel}` и
`POST .../notifications/{channel}/test`, где канал называется в пути, оба случая отвечают
одинаково — `404`. `GET .../notifications` параметра `{channel}` не принимает и `404` вернуть не
может в принципе; там та же гарантия достигается иначе — недоступный или несуществующий канал
просто не попадает в список `200 { "channels": [...] }` (см. выше: «канал, которого в списке нет,
для этого тенанта не существует»). В обоих случаях цель одна: разные ответы позволили бы
администратору пространства перебором получить полный список каналов продукта, включая
невыданные.

Секрет канала наружу не возвращается никогда. В ответах есть только `secret_hint` — маска вида
`••••4821`. Пустой `secret` в `PUT` означает «не менять сохранённый», а не «стереть» (см. выше).
...
- `GET /api/v1/system/notification-channels` — каналы уведомлений, собранные в этот бинарь
  (`app.Config.NotificationChannels`), а не только выданные какому-то тенанту → `200
  { "channels": [...] }`, где элемент — `{ "name", "title",
  "entitlement_key" }`. `entitlement_key` — bare-ключ вида `notifications.<канал>`, который панель
  отправляет в `PUT .../tenants/{id}/entitlements`, чтобы выдать канал тенанту. Read-only.
  ...
  "items": [
    {
      ...
      "url": "/?team=4&period=4&goal=42",
      "context": { "team": "Компания / Платформа / Биллинг", "goal": "Снизить отток" }
    }
  ],
...
- `title`/`body` рендерятся **на сервере** (`internal/render/notify`) из `kind` + `payload` —
  ...
  собирает текст сам;
- `subject` — необязательное поле: название сущности, о которой уведомление, отдельной строкой
  в карточке. Отдаётся только для тех `kind`, чьё тело сущность не называет; сегодня это один
  `kr_checked_in` (его тело — строка статуса и прогресса по пяти правилам, места названию KR в
  ней нет, и без `subject` в команде с десятком KR непонятно, о каком именно чек-ин). Решает
  сервер, а не клиент; при пустом значении поле в ответе отсутствует;
- `context` — необязательный объект: где произошло событие. `team` — путь команды от корня,
  через ` / ` («Компания / Платформа / Биллинг»); `goal` — название цели. Нужен, чтобы длинный
  список читался без открытия каждой записи. Обе половины резолвятся **на чтении**, в том же
  запросе, что и актор (рекурсивный подъём по `teams.parent_id`, ограниченный `tenant_id` на
  каждом шаге, и `LEFT JOIN goals`) — запрос на строку был бы N+1 на странице до 100 записей.
  Значения не заморожены на момент записи: переименованная команда или цель показывается
  текущим именем, потому что ориентируются по актуальному дереву. Объект отсутствует целиком,
  когда у уведомления нет ни команды, ни цели (или цель удалена); половина контекста законна;
...
- `url` — куда ведёт клик; пусто, если у уведомления нет цели-якоря (уведомление всё равно
  рендерится, просто некликабельно). Формат — канонический deep-link трекера
  `/?team=&period=&goal=&kr=&comment=` (`030-user-flows.md`, «Deep-link в трекер»): имена и
  порядок параметров те же, что строит `buildTargetURL` в `web/static/ui.js` и читает
  `readURLNav` в `web/static/tracker.js`. Другие имена (`goal_id`, `team_id`, `period_id`)
  трекер не читает вовсе — клик по колокольчику тогда никуда не ведёт.
...
- upsert KR note — `POST /api/v1/krs/{krID}/note` (маршрут существует; в этом репозитории у него нет известных клиентов — интерфейс правит заметку через поле `note` в теле запроса прогресса, см. ниже)
...
- delete one notification — `DELETE /api/v1/notifications/{id}`
- update notification preferences — `PUT /api/v1/notifications/preferences`

### `DELETE /api/v1/notifications/{id}`

Назначение: убрать одну запись из колокольчика — кнопка справа от уведомления. Отличается от
пометки прочитанным: прочитанное остаётся в списке до истечения срока хранения, удалённое
исчезает сразу.
...
Удаление ограничено И тенантом, И получателем: строка ищется по `id`, `tenant_id` и `user_id`
из сессии. Без условия по `user_id` участник того же пространства удалял бы чужие уведомления,
зная идентификатор.

Errors:
- `400 VALIDATION_ERROR` — `id` не число или не положителен
- `403 FORBIDDEN` — нет активного пространства
- `404 NOT_FOUND` — уведомления нет ИЛИ оно принадлежит другому пользователю. Оба случая дают
  **неотличимый** ответ (тот же код и то же тело) намеренно: разные ответы позволили бы
  перебором идентификаторов подтверждать существование чужих уведомлений.

Side effects: если удалялось непрочитанное, счётчик `GET /api/v1/notifications/unread-count`
уменьшается — он считает строки, а не отдельный счётчик, поэтому расхождения быть не может.
...
## TODO: два конверта ошибки

В API сейчас живут два несовместимых формата ошибки:

```json
{"error": {"code": "VALIDATION_ERROR", "message": "...", "fields": {...}}}   // v1.WriteError
{"error": "invalid body"}                                                    // admin/system/onboarding
```

Структурный конверт отдаёт основной `/api/v1` (`v1.WriteError`); плоский — планы
`admin`, `system` и `onboarding` через собственные `WriteError` в своих
`*common`-пакетах. Клиент вынужден разбирать оба, а машинного кода ошибки на
админских эндпоинтах нет вовсе — UI различает случаи по тексту сообщения.

Приведение к одному конверту — ломающее изменение контракта: нужно синхронно
править фронтенд, поэтому вынесено за скоуп раскладки обработчиков. Тестовый
хелпер `handlertest` принимает оба формата и помечен ссылкой на этот пункт,
чтобы разница не выглядела нормой.
...
- порядок внутри одной операции чек-ина: сначала применяется обновление прогресса, затем — если `health_status` передан — ручной статус. Авто-`done` при переходе прогресса `<100% → =100%` (**однократно**, если статус ещё не `done`) срабатывает только тогда, когда `health_status` в запросе **не** передан, поэтому ручной статус всегда побеждает авто-`done` (гарантия «ручной побеждает»);
- доступ — как у обновления прогресса (доступ к команде-владельцу); health-статус в расчёт прогресса не входит.

Те же три эндпоинта дополнительно принимают опциональное поле `"note"` (string) — заметку KR, применяемую тем же запросом, что и прогресс:

- поле **опционально**: отсутствует в теле запроса → заметка не трогается; присутствует, включая пустую строку `""`, → заметка выставляется в переданное значение — здесь, в отличие от `health_status` выше, разница «поле не пришло» / «пришла пустая строка» значима. Пустая строка стирает **текст** заметки, но не саму запись: `key_results[].note` остаётся объектом с `text: ""`, обновлённым автором и `updated_at` — удалить заметку через API по-прежнему нельзя (инвариант KeyResultNote в `020-domain-model.md`);
- прогресс, health-статус и заметка одного запроса применяются одной операцией чек-ина (`CheckIn` в usecase-слое ключевых результатов) и порождают не более одного события шины на запрос — вместо прежних раздельных запросов на прогресс и на заметку, каждый со своим событием;
- доступ — как у обновления прогресса.

Отдельный `POST /api/v1/krs/{krID}/note` (см. выше) по-прежнему принимает только `{ "text": <string> }` и, в отличие от поля `note` у progress-эндпоинтов, требует непустой (после trim) текст — `400 VALIDATION_ERROR`, если пусто; предназначен для правки заметки без чек-ина, если такой путь понадобится клиенту вне интерфейса трекера.
