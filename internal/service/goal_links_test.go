...
func newLinkFixture(t *testing.T) (*linkFixture, func()) {
	...
	f := &linkFixture{
		...
		svc: New(Deps{
			Goals:     gr,
			GoalLinks: goallinks.NewGoalLinkRepository(pool),
...
func TestSetGoalParents_ValidatesAccessAndCycle(t *testing.T) {
	...
	if err := f.svc.SetGoalParents(f.ctx, f.scope, []int64{teamA}, false, child, []int64{parent}, 1); err != ErrGoalLinkNotAccessible {
		t.Fatalf("scoped err = %v, want ErrGoalLinkNotAccessible", err)
	}

	// Scope on both → ok.
	if err := f.svc.SetGoalParents(f.ctx, f.scope, []int64{teamA, teamB}, false, child, []int64{parent}, 1); err != nil {
		t.Fatalf("link both: %v", err)
	}

	// Self-link.
	if err := f.svc.SetGoalParents(f.ctx, f.scope, []int64{teamA}, false, child, []int64{child}, 1); err != ErrGoalLinkSelf {
		t.Fatalf("self err = %v, want ErrGoalLinkSelf", err)
	}

	// Cycle: parent -> child (reverse) with scope on both.
	if err := f.svc.SetGoalParents(f.ctx, f.scope, []int64{teamA, teamB}, false, parent, []int64{child}, 1); err != ErrGoalLinkCycle {
		t.Fatalf("cycle err = %v, want ErrGoalLinkCycle", err)
	}
}
...
func TestSetGoalParents_AdminAllBypassesScope(t *testing.T) {
	...
	if err := f.svc.SetGoalParents(f.ctx, f.scope, nil, true, child, []int64{parent}, 1); err != nil {
		t.Fatalf("admin link: %v", err)
	}
	parents, _, err := f.svc.ListLinksForGoals(f.ctx, f.scope, []int64{child}, nil, true)
	if err != nil {
		t.Fatalf("list: %v", err)
	}
...
func TestSetGoalParents_RecordsActivity(t *testing.T) {
	...
	if err := f.svc.SetGoalParents(f.ctx, f.scope, []int64{team}, false, child, []int64{p1}, 1); err != nil {
		t.Fatalf("link: %v", err)
	}
	// Replace with empty → goal_unlinked.
	if err := f.svc.SetGoalParents(f.ctx, f.scope, []int64{team}, false, child, nil, 1); err != nil {
