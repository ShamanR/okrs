...
// A user whose ACTIVE tenant role is not admin gets no unrestricted access — scope comes
// strictly from that tenant's hierarchy grants.
func TestLoadScopeNonAdminGetsGrantExpansion(t *testing.T) {
	e := NewPolicyEvaluator(fakeGrants{
		byUser:      map[int64][]grants.HierarchyGrant{7: {{TeamID: 10}}},
		descendants: map[int64][]int64{10: {10, 11}},
	}, nil)
	user := &domain.User{ID: 7} // not a tenant admin
	...
	if !ok || ids == nil {
		t.Fatalf("non-admin must not get unrestricted access; ids=%v ok=%v", ids, ok)
	}
	if len(ids) != 2 {
