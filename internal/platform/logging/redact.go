...
//
// Буквы — \p{L}, а не только латиница, и TLD от одной буквы: провайдер может
// выдать интернационализованный адрес или короткий домен, а приложение его
// не валидирует. От версий и путей вроде foo@v1.2.3 шаблон отделяет требование
// букв в TLD.
//
// Это ВТОРОЙ рубеж, а не гарантия: множество принимаемых адресов шире любого
// разумного шаблона. Гарантию даёт то, что адрес вообще не попадает в текст:
// см. mattermost.sender.call, который пишет в ошибку метку эндпоинта, а не путь.
var emailPattern = regexp.MustCompile(`[\p{L}\p{N}._+\-]+(?:@|%40)[\p{L}\p{N}.\-]+\.\p{L}+`)
