...
const uri = "/api/v1/admin/members/7"

func TestRequiresTenant(t *testing.T) {
	w := handlertest.Do(New(&fakeOnboard{}).Delete, http.MethodPost, uri, "",
...
func TestAppliesPackageOperation(t *testing.T) {
	f := &fakeOnboard{}
	w := handlertest.Do(New(f).Delete, http.MethodPost, uri, "",
		handlertest.Tenant(1), handlertest.URLParam("userID", "7"))
	handlertest.Status(t, w, http.StatusNoContent)
	if f.called != "remove" {
		t.Fatalf("вызвана операция %q, want %q", f.called, "remove")
	}
	if f.userID != 7 {
