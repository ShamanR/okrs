...
const uri = "/api/v1/system/settings/no-access-message"
...
func TestStoresUnderFixedKey(t *testing.T) {
	f := newFakeSettings()
	w := handlertest.Do(New(f).Put, http.MethodPut, uri, `{"message":"нет доступа"}`)
	handlertest.Status(t, w, http.StatusNoContent)
	if _, ok := f.written["no_access_message"]; !ok {
		t.Fatalf("ключ %q не записан, записано: %v", "no_access_message", f.written)
	}
}
