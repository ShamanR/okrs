...
func TestGatePostBadGoalID(t *testing.T) {
	w := handlertest.Do(New(nil, nil, nil).Post, http.MethodGet, "/api/v1/goals/{goalID}/comments", "",
...
func TestGateDeleteBadGoalID(t *testing.T) {
	w := handlertest.Do(New(nil, nil, nil).Delete, http.MethodGet, "/api/v1/goals/{goalID}/comments/{commentID}", "",
		handlertest.Tenant(1),
		handlertest.URLParam("goalID", "не-число"),
		handlertest.URLParam("commentID", "1"))
	handlertest.IsError(t, w, http.StatusBadRequest)
}
