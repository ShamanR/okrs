...
func TestGatePostBadKrID(t *testing.T) {
	w := handlertest.Do(New(nil, nil, nil).Post, http.MethodGet, "/api/v1/krs/{krID}/progress/boolean", "",
...
func TestPostAppliesProgressHealthAndNote(t *testing.T) {
	...
	store.KeyResults[7] = domain.KeyResult{ID: 7, GoalID: 1, Kind: domain.KRKindBoolean}
	h := newTestHandler(store)

	body := `{"done":true,"health_status":"done","note":"готово"}`
	w := handlertest.Do(h.Post, http.MethodPost, "/api/v1/krs/{krID}/progress/boolean", body,
		handlertest.Tenant(1), handlertest.UserID(5, "u5"), handlertest.URLParam("krID", "7"))
	handlertest.Status(t, w, http.StatusOK)

	if !store.BooleanUpdates[7] {
		t.Fatalf("прогресс не применился: %+v", store.BooleanUpdates)
	}
	if store.HealthUpdates[7] != domain.KRHealthDone {
		t.Fatalf("health-статус не применился: %+v", store.HealthUpdates)
	}
	if store.Notes[7].Text != "готово" {
...
func TestPostInvalidHealthStatusStill400(t *testing.T) {
	...
	body := `{"done":true,"health_status":"bogus"}`
	w := handlertest.Do(h.Post, http.MethodPost, "/api/v1/krs/{krID}/progress/boolean", body,
		handlertest.Tenant(1), handlertest.UserID(5, "u5"), handlertest.URLParam("krID", "7"))
	handlertest.IsError(t, w, http.StatusBadRequest)

	if store.BooleanUpdates[7] {
		t.Fatalf("невалидный health_status не должен допускать обновление прогресса: %+v", store.BooleanUpdates)
	}
}
...
func TestPostNormalizesNoteCRLF(t *testing.T) {
	...
	body := `{"done":true,"note":"первая строка\r\nвторая строка"}`
	w := handlertest.Do(h.Post, http.MethodPost, "/api/v1/krs/{krID}/progress/boolean", body,
		handlertest.Tenant(1), handlertest.UserID(5, "u5"), handlertest.URLParam("krID", "7"))
	handlertest.Status(t, w, http.StatusOK)
...
func TestPostPublishesHealthThroughBus(t *testing.T) {
	store := servicetest.NewStore()
	store.KeyResults[7] = domain.KeyResult{ID: 7, GoalID: 1, Kind: domain.KRKindBoolean, HealthStatus: domain.KRHealthAtRisk}
	h, bus := newTestHandlerWithBus(store)

	body := `{"done":true,"health_status":"on_track"}`
	w := handlertest.Do(h.Post, http.MethodPost, "/api/v1/krs/{krID}/progress/boolean", body,
		handlertest.Tenant(1), handlertest.UserID(5, "u5"), handlertest.URLParam("krID", "7"))
	handlertest.Status(t, w, http.StatusOK)
