...
// Тест переехал из internal/service при выделении слоя usecase.
...
func TestExportOKRTree(t *testing.T) {
	...
	res2, err := svc.ExportOKR(ctx, scope, exportuc.ExportParams{
		TeamID: rootID, PeriodID: periodID, Scope: export.ScopeTree,
	...
	res3, err := svc.ExportOKR(ctx, scope, exportuc.ExportParams{
	...
	res5, err := svc.ExportOKR(ctx, scope, exportuc.ExportParams{
	...
	res6, err := svc.ExportOKR(ctx, scope, exportuc.ExportParams{
		TeamID: rootID, PeriodID: periodID, Scope: export.ScopeTree,
		Options: export.Options{Format: export.FormatShort}, AllowedTeamIDs: []int64{rootID, childID},
	})
	if err != nil {
	...
	res4, err := svc.ExportOKR(ctx, scope, exportuc.ExportParams{
